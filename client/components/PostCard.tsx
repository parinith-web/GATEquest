    ...
    <article className="flex flex-col border border-pulse-border rounded-lg bg-pulse-card overflow-hidden">
      ...
      <div className="px-4 pb-4 flex flex-col gap-[7px]">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {isVerifiedEducator && (
            <span className="text-[10px] font-mono font-bold text-pulse-blue px-1.5 py-0.5 rounded-sm border border-pulse-blue/30 bg-pulse-blue/10 uppercase tracking-wider whitespace-nowrap">
              VERIFIED EDUCATOR
            </span>
          )}
          <span className="text-[12px] font-mono text-pulse-muted">•</span>
          <span className="text-[12px] font-mono text-pulse-muted">{timeAgo}</span>
          <div className="ml-auto flex items-center gap-1 flex-wrap justify-end">
            {tags.map((tag) => (
              <span
                key={tag}
                className="text-[10px] font-mono text-pulse-muted px-2 py-0.5 rounded-sm border border-pulse-border2 bg-pulse-border uppercase tracking-[0.5px]"
              >
                {tag}
              </span>
            ))}
        ...
        {/* Title */}
        <h2 className="text-[17px] font-sans font-semibold text-pulse-blue leading-[1.5] pt-1">
          {title}
        </h2>

        {/* Body */}
        <p className="text-[14px] font-sans text-pulse-muted leading-[1.63]">
          {body}
        </p>
        ...
        <div className="flex items-center gap-4 pt-3 mt-1 border-t border-pulse-border flex-wrap">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={cn(
                "transition-colors",
                upvoted ? "text-pulse-blue" : "text-pulse-dim hover:text-pulse-blue"
              )}
            >
              <svg width="12" height="8" viewBox="0 0 12 8" fill="none">
                <path
                  d="M6 2.8L1.4 7.4L0 6L6 0L12 6L10.6 7.4L6 2.8Z"
                  fill="currentColor"
                />
              </svg>
            </button>
            <span
              className={cn(
                ...
                upvoted ? "text-pulse-blue" : "text-pulse-text"
              )}
            >
              {votes}
            </span>
